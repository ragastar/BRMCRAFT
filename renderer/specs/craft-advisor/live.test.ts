...
import {
  buildReferenceOpts,
  fetchInBatches,
} from "@/web/craft-advisor/trade-reference";
...
describe.skipIf(!LIVE)(
  "LIVE harness — шаблон базы по сравнимым кольцам (CRAFT_LIVE=1)",
  () => {
      ...
      it(`pipeline: ${name}`, async () => {
        ...
        try {
          ...
          const statIds = (preset.stats ?? [])
            .map((s: any) => s.tradeId?.[0])
            .filter(
              (id: string) => id && /^(explicit|implicit|pseudo)\./.test(id),
            );
          if (statIds.length >= 2) {
            body.query.stats = [
              {
                type: "count",
                value: { min: 2 },
                filters: statIds.map((id: string) => ({ id })),
              },
            ];
          }
          out.statIds = statIds;
